...
# HACK HACK HACK!!! Python 3.14 multiprocessing compatibility
...
def setup_config(args):
    ...
    for preinstalled_images in preinstalled_images_paths:
        if os.path.isdir(preinstalled_images):
            if os.path.isfile(preinstalled_images + "/system.img") and os.path.isfile(preinstalled_images + "/vendor.img"):
                ...
                logging.warning(
                    "Found directory {} but missing system or vendor image, ignoring...".format(preinstalled_images)
                )
    ...
    args.system_ota = (
        args.system_channel + "/" + args.rom_type +
    ...
    if not args.vendor_type:
        raise ValueError("Failed to get vendor OTA channel: {}".format(vendor_ota))
...
class DbusInitializer(dbus.service.Object):
    ...
    def Init(self, params, sender=None, conn=None):
        ...
        channels_cfg = tools.config.load_channels()
        no_auth = (
            params["system_channel"] == channels_cfg["channels"]["system_channel"] and
